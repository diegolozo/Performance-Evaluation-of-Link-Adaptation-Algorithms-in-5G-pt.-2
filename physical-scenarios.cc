...
DefaultPhysicalDistribution(ns3::NodeContainer& gnbNodes, ns3::NodeContainer& ueNodes, double mobility)
{
    ...
    NS_LOG_INFO( "- Positioning Nodes" );
    Ptr<ListPositionAllocator> gnbPositionAlloc = CreateObject<ListPositionAllocator>();
    for (uint32_t u = 0; u < gnbNodes.GetN(); ++u)
    {
        NS_LOG_INFO( "- Node ID " << gnbNodes.Get (u)->GetId() << " Sys ID:" << gnbNodes.Get(u)->GetSystemId() + u );
        NS_LOG_INFO( "- gNb: " << u << "\t" << "(" << gNbX << "," << gNbY+gNbD*u <<")" );
    ...
    for (uint32_t u = 0; u < ueNodes.GetN(); ++u)
    {
        ...
        NS_LOG_INFO( "- UE: " << u << "\t" << "Pos: "<<"(" << xx << "," << yy <<")" << "\t" << "Speed: (" << speed << ", 0)" );
    ...
    if (enableBuildings)
    {
        ...
        double buildLy=8;  // Y Length


        std::cout << TXT_CYAN << "Installing Building" << TXT_CLEAR << std::endl;
        NS_LOG_INFO( "- Installing Building" );
        ...
        gridBuildingAllocator->SetBuildingAttribute("ExternalWallsType", EnumValue(Building::ConcreteWithoutWindows)); // Building::Custom   Building::ConcreteWithoutWindows
    ...
    if (enableTrees){
        ...
        uint32_t treegridWidth = (int)(numOfTrees/2) ;//

        double treeLx = 3 ;  // X Length
        double treeLy = 3 ;  // Y Length
        double treeLz = 3 ;  // Z Length
        double treeDx = 8 - treeLx/2 ;  // Distance X between trees
        double treeDy = 8 - treeLy/2 ;  // Distance Y between trees
        double treeX = gNbX - treeLx/2  - (int)(treegridWidth/2)*(treeDx + treeLx); // Initial X Position
        double treeY = 15 - treeLy/2 ; // Initial Y Position
        
        NS_LOG_INFO( "- Installing Trees" );
    ...
    NS_LOG_INFO( "OK Installed default distribution" );
...
TreePhysicalDistribution(ns3::NodeContainer& gnbNodes, ns3::NodeContainer& ueNodes, double mobility)
{
    ...
    for (uint32_t u = 0; u < ueNodes.GetN(); ++u)
    {
        NS_LOG_INFO( "- UE: " << u << "\t" << "Pos: "<<"(" << xUE << "," << yUE +ueDistance*u <<")" << "\t" << "Speed: (" << speed << ", 0)" );
    ...
    if (enableBuildings)
    {
        ...
        std::cout << TXT_CYAN << "OK Installed Tree distribution." << TXT_CLEAR << std::endl;
    }
