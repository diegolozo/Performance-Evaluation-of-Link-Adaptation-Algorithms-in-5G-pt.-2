...
NrMacSchedulerLCG::AssignedData(uint8_t lcId, uint32_t size, std::string type)
{
                ...
                // data to be transmitted
    {
        ...
        if (size - rlcOverhead >= m_lcMap.at(lcId)->m_rlcTransmissionQueueSize)
        {
            ...
            // m_lcMap.at(lcId)->m_rlcTransmissionQueueSize = 0; // JSA Commented
            // JSA. commented by Goodsol.. this line prevents the next transmission when all HARQ retx fails. However, note that we can still make lcg queue as 0 with zero BSR reception
            NS_LOG_DEBUG(Simulator::Now().GetSeconds()<<" Queue: "<<m_lcMap.at(lcId)->m_rlcTransmissionQueueSize); // JSA Added
            m_lcMap.at(lcId)->m_rlcTransmissionQueueSize = 20; // JSA Added
