...
{
  // NS_LOG_FUNCTION (this << (uint64_t) m_frameType); // JSA Commented
  NS_ASSERT (m_frameType >= PADDING and m_frameType <= STREAM111);

  os << "|" << FrameTypeToString () << "|"; // JSA final "\n" removed
  switch (m_frameType)
    {
      ...
      case RST_STREAM:
        ...
        os << "|Stream Id " << m_streamId << "|";               // JSA final "\n" removed
        os << "|Application Error Code " << m_errorCode << "|"; // JSA final "\n" removed
        os << "|Final Offset " << m_offset << "|";            // JSA final "\n" removed
        break;

      case CONNECTION_CLOSE:

        os << "|Application Error Code " << TransportErrorCodeToString () << "|"; // JSA final "\n" removed
        os << "|Reason Phrase Length " << m_reasonPhraseLength << "|";            // JSA final "\n" removed
        os << "|Reason Phrase ";
        for (auto& elem : m_reasonPhrase)
          {
            os << elem;
      ...
      case MAX_DATA:
        ...
        os << "|Maximum Data " << m_maxData << "|"; // JSA final "\n" removed
        break;

      case MAX_STREAM_DATA:

        os << "|Stream Id " << m_streamId << "|";   // JSA final "\n" removed
        os << "|Maximum Stream Data " << m_maxStreamData << "|";  // JSA final "\n" removed
        break;

      case MAX_STREAM_ID:
        os << "|Maximum Stream Id " << m_maxStreamId << "|";      // JSA final "\n" removed
        break;
      ...
      case NEW_CONNECTION_ID:
        ...
        os << "|Sequence " << m_sequence << "|";          // JSA final "\n" removed
        os << "|Connection Id " << m_connectionId << "|"; // JSA final "\n" removed
        //os << "|Stateless Reset Token " << m_statelessResetToken << "|";  // JSA final "\n" removed
        break;
      ...
      case ACK:
        ...
        os << "|Largest Acknowledged " << m_largestAcknowledged << "|"; // JSA final "\n" removed
        os << "|Ack Delay " << m_ackDelay << "|";                       // JSA final "\n" removed
        os << "|Ack Block Count " << m_ackBlockCount << "|";            // JSA final "\n" removed
        os << "|First Ack Block " << m_firstAckBlock << "|";            // JSA final "\n" removed
        for (uint64_t j = 0; j < m_ackBlockCount; j++)
          {
            os << "|Gap " << m_gaps[j] << "|";                                    // JSA final "\n" removed
            os << "|Additional Ack Block " << m_additionalAckBlocks[j] << "|";    // JSA final "\n" removed
          }
        break;

      case PATH_CHALLENGE:

        os << "|Data " << (uint64_t)m_data << "|";   // JSA final "\n" removed
        break;
