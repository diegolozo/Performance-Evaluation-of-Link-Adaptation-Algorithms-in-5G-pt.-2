...
QuicHeader::Print (std::ostream &os) const
{
  ...
  os << TypeToString () << "||"; // JSA middle "\n" removed

  if (HasConnectionId ())
    {
      os << "ConnectionID " << m_connectionId << "||"; // JSA middle "\n" removed
    }
  if (IsShort ())
    {
      os << "PacketNumber " << m_packetNumber << "|"; // JSA Final "\n" removed
    }
  else
    {
      os << "Version " << (uint64_t)m_version << "|"; // JSA Final "\n" removed
      os << "PacketNumber " << m_packetNumber << "||"; // JSA middle "\n" removed
    }
