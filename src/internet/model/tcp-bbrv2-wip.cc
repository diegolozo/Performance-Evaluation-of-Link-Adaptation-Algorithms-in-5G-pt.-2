    ...
    static TypeId tid =
        TypeId("ns3::TcpBbr")
            ...
            .AddAttribute("BBRVariant",
                          ...
                          MakeEnumAccessor<TcpBbr::BbrVar>(&TcpBbr::m_variant),
...
TcpBbr::CheckDrain(Ptr<TcpSocketState> tcb)
{
    ...
    {
        m_cycleIndex = BbrBwPhase::BBR_BW_PROBE_NS; // Is this ok? - AGDT
        EnterProbeBW(tcb);
    }
}
...
TcpBbr::UpdateRound(Ptr<TcpSocketState> tcb, const TcpRateOps::TcpRateSample& rs)
{
    ...
    {
        m_nextRoundDelivered = m_delivered;
        // m_roundCount++; // JSA Commented
        // JSA https://groups.google.com/g/ns-3-users/c/9dSUtfQWZ7E/m/waNsS7uCBQAJ
        // JSA if sentence added
        if((GetBbrState() == BbrMode_t::BBR_PROBE_BW || GetBbrState() == BbrMode_t::BBR_STARTUP) )
        {
			m_roundCount++;
        }
        m_roundStart = true;
...
TcpBbr::UpdateBottleneckBandwidth(Ptr<TcpSocketState> tcb, const TcpRateOps::TcpRateSample& rs)
{
    ...
    // if (rs.m_deliveryRate == 0) // JSA original commented
    if (rs.m_delivered < 0 || rs.m_interval == Seconds(0)) // JSA: https://gitlab.com/nsnam/ns-3-dev/-/merge_requests/1087
    {
        return;
    }
    ...
    // }
    // JSA changed by neaset if sentence
    if(GetBbrState() == BbrMode_t::BBR_PROBE_BW || GetBbrState() == BbrMode_t::BBR_STARTUP )
    {
        if ( (rs.m_deliveryRate >= m_maxBwFilter.GetBest() || !rs.m_isAppLimited) && (tcb->m_congState != TcpSocketState::CA_LOSS) && (tcb->m_congState != TcpSocketState::CA_RECOVERY)) //(GetBbrState() != BbrMode_t::BBR_P>
        {
            m_maxBwFilter.Update(rs.m_deliveryRate, m_roundCount);
        }
    }
...
TcpBbr::UpdateControlParameters(Ptr<TcpSocketState> tcb, const TcpRateOps::TcpRateSample& rs)
{
    ...
    if (m_variant == BbrVar::BBR_V2)
    {
        ...
        m_ecnInCycle |= m_deliveredEce_rs > 0;
    }
...
TcpBbr::CwndEvent(Ptr<TcpSocketState> tcb, const TcpSocketState::TcpCAEvent_t event)
{
    ...
    {
        m_deliveredEce += tcb->m_lastAckedSackedBytes;
        if (tcb->m_txItemDelivered > tcb->m_rs.m_priorDelivered)
        {
            tcb->m_rs.m_priorDeliveredEce = tcb->m_txItemDelivered;
        }
        m_deliveredEce_rs = tcb->m_lastAckedSackedBytes - tcb->m_rs.m_priorDeliveredEce;
    }
    else if (event == TcpSocketState::CA_CWR)
    {
        m_isEce = true;
    }
}
