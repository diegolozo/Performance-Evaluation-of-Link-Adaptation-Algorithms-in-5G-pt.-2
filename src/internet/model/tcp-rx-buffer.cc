...
// JSA: Added to trace TCP buffer
std::ofstream m_TcpRxBufferStatFile;
std::string m_TcpRxBufferStatFileName;
...
TcpRxBuffer::Add(Ptr<Packet> p, const TcpHeader& tcph)
{
    ...
    // JSA
    uint16_t destinationPort = tcph.GetDestinationPort();
    if (!m_TcpRxBufferStatFile.is_open())
    {
        std::ostringstream oss;
        oss <<"TcpBufferStat.txt";
        m_TcpRxBufferStatFileName = oss.str();
        m_TcpRxBufferStatFile.open(m_TcpRxBufferStatFileName.c_str());
        m_TcpRxBufferStatFile << "Time"
                              << "\t"
                              << "Port"
                              << "\t"
                              << "txBufferSize" // bytes in buffer
                              << "\t"
                              << "PacketSize" // bytes current paquet
                              << "\t"
    ...
    if (headSeq >= tailSeq)
    {
        ...
        // JSA: Buffer full?
        m_TcpRxBufferStatFile << Simulator::Now().GetSeconds()
                            << "\t"
                            << destinationPort // p->GetUid()
                            << "\t"
                            << m_size
                            << "\t"
                            << pktSize
