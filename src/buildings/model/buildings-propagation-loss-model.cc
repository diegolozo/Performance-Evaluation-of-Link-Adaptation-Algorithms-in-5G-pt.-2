...
BuildingsPropagationLossModel::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::BuildingsPropagationLossModel")
            ...
            .AddAttribute("ExternalWallLoss",
                          "Additional loss for custom external wall [dB]",
                          DoubleValue(15.0),
                          MakeDoubleAccessor(&BuildingsPropagationLossModel::m_lossExternalWall),
                          MakeDoubleChecker<double>());
...
BuildingsPropagationLossModel::ExternalWallLoss(Ptr<MobilityBuildingInfo> a) const
{
    ...
    else if (aBuilding->GetExtWallsType() == Building::Custom)
    {
        loss = m_lossExternalWall;
    }
